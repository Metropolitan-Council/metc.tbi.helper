...
tbi$hh[, scaled_weight := fcase(
  survey_year == 2021, hh_weight * 7905 / 7516,
...
hh_size <- tbi$hh[
  ...
  hh_weight != 0 | NA, sum(hh_weight),
  keyby = .(num_people, survey_year)
] %>%
...
hh_size[
  , .SD %>%
    ...
    layout(
      xaxis = list(title = "sum household weight"),
      yaxis = list(title = NA)
    )
]
...
# mean scaled weight
hh_size <- tbi$hh[
  hh_weight != 0 | NA, mean(scaled_weight),
  keyby = .(num_people, survey_year)
] %>%
...
## income (detailed)
hh_income <- tbi$hh[
  hh_weight != 0 | NA, .(sum(hh_weight), .N),
...
# max scaled weight
hh_income <- tbi$hh[
  hh_weight != 0 | NA, max(scaled_weight),
  keyby = .(income_detailed, survey_year)
] %>%
  setnames("V1", "max_hh_wt")
...
# mean scaled weight
hh_income <- tbi$hh[
  hh_weight != 0 | NA, .(mean(scaled_weight), .N),
  keyby = .(income_detailed, survey_year)
] %>%
  setnames("V1", "avg_hh_wt")
...
hh_vehicles <- tbi$hh[
  ...
  hh_weight != 0 | NA, sum(hh_weight),
  keyby = .(num_vehicles, survey_year)
] %>%
  setnames("V1", "sum_hh_wt")
