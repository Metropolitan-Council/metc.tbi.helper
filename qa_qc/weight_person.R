...
#---- person gender ----
p_gender <- tbi$person[
  person_weight != 0|NA, sum(person_weight), keyby = .(gender, survey_year)
] %>%
...
p_gender[
  , .SD %>%
    ...
    layout(xaxis = list(title = "sum person weight"),
           yaxis = list(title = NA))
]
...
#---- age ----
p_age <- tbi$person[
  person_weight != 0|NA, sum(person_weight), keyby = .(age, survey_year)
] %>%
  setnames("V1", "sum_person_wt")
...
#---- employment ----
p_employ <- tbi$person[
  person_weight != 0|NA, sum(person_weight), keyby = .(employment, survey_year)
] %>%
  setnames("V1", "sum_person_wt")
...
#---- student status ----
p_stu <- tbi$person[
  person_weight != 0|NA, sum(person_weight), keyby = .(student, survey_year)
] %>%
  setnames("V1", "sum_person_wt")
...
#---- education ----
p_edu <- tbi$person[
  person_weight != 0|NA, sum(person_weight), keyby = .(education, survey_year)
] %>%
  setnames("V1", "sum_person_wt")
