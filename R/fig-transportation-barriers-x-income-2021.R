...
barrierdat %>%
  ...
  labs(x = "Household Income", y = str_wrap("Percent of Population", width = 10),
       title = str_wrap(width = 50,
...
barrierdat %>%
  mutate(ethnicity_afam = recode_factor(ethnicity_afam, "Yes" = "Yes",
                                        "Not selected" = "No")) %>%
  as_survey_design(w = person_weight) %>%
  ...
  scale_x_discrete(labels = function(x) str_wrap(x, width = 10)) +
  scale_y_continuous(limits = c(0, 0.1), labels = scales::percent) +
  labs(x = "Race/Ethnicity: Black, African, or African American", y = str_wrap("Percent of Population", width = 10),
