# packages-------------------------------------------
packages <- list("bit64", "dplyr", "ggplot2", "plotly", "srvyr", "councilR", "sysfonts", "showtext")
...
plotdat <-
  ...
  mutate(mytext2 = "of adults") %>%
  mutate(mytext_EV1 = paste0(
    round(pct_adults),
    "%"
  )) %>%
  mutate(mytext_EV2 = "of adults") %>%
...
plot <-
  plotdat %>%
  ggplot(aes(x = gal_bins, y = n_adults, fill = gal_bins)) +
  geom_bar(stat = "identity") +
  geom_errorbar(
    aes(
      ymin = n_adults - n_adults_se,
      ymax = n_adults + n_adults_se
    ),
    width = 0,
    color = "black"
  ) +
  ...
  geom_text(
    aes(label = mytext1, y = n_adults - n_adults_se - 1e4),
  ...
  geom_text(
    aes(label = mytext2, y = n_adults - n_adults_se - 1e4),
  ...
  geom_text(
    aes(label = mytext_EV1, y = n_adults + n_adults_se + 5.5e4),
    size = unit(8, units = "line"),
    # lineheight = 0.25,
    family = "Arial Narrow Italic",
    vjust = 1,
  ...
  geom_text(
    aes(label = mytext_EV2, y = n_adults + n_adults_se + 4.5e4),
    size = unit(4, units = "line"),
    # lineheight = 0.25,
    family = "Arial Narrow Italic",
    vjust = 3,
  ...
  labs(
    title = "Most residents use less than a gallon of gas per day",
    ...
    caption = stringr::str_wrap(
      ...
      paste0(
        ...
        "Data are weighted to reflect the average weekday of a metro resident. ",
...
# save plot ----------------------
jpeg("fig/gallons-consumed-perperson.jpeg", width = 1200, height = 800, units = "px")
plot
dev.off()

png("fig/gallons-consumed-perperson.png", width = 1200, height = 800, units = "px")
plot
dev.off()
