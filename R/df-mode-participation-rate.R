...
no_travel_share <- 
  tbi$day %>% select(person_id, day_num, day_weight, num_trips) %>%
  mutate(zero_trips = case_when(num_trips == 0 ~ 1,
                                num_trips > 0 ~ 0)) %>%
  # only weekdays; only complete survey days: 
  filter(day_weight > 0) %>%
  # filter to adults:
...
# For Eric
datlist <- list("transit_frequency_selfreported" = transit_freq_self_reported,
