...
median_mpg <- tbi19$veh %>%
  left_join(tbi19$hh %>% select(hh_id, hh_weight)) %>%
...
driver_trips %>%
  left_join(tbi19$veh, by = c("hh_id", "veh_id")) %>%
...
gal_per_hh_day <-
  ...
  ungroup() %>%
  left_join(tbi19$day %>% select(person_id, day_num, day_weight),
